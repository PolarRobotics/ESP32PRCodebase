 ...
 */
uint8_t SerialPWM::attach(int pin) {
 ...
 * @return uint8_t the channel number the pin is attached to, 255 if failure
 */
uint8_t SerialPWM::attach(int pin, int min, int max) {
...
*/
void SerialPWM::write(float pwr) {
    analogWrite(motors[this->motorIndex].pin, power2Duty(pwr));
    // ledcWrite(this->motorIndex, power2Duty(pwr));
}

void SerialPWM::displayPinInfo() {
...
*/
uint16_t SerialPWM::power2Duty(float power) {
 ...
 * 
 */
void SerialPWM::writelow() {
    // digitalWrite(motors[this->motorIndex].pin, LOW);
    analogWrite(motors[this->motorIndex].pin,0);
    // write(0);
}